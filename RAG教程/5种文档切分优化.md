...
# 延迟切块 Late-chunking
- **切块原理**：整篇先转token➡ embedding → 再根据 token 位置切块
- **解决问题**：普通切块对每个切块单独embedding看不到上下文的问题
- **核心逻辑**：先确定 chunk 文本和它在全文 token 序列里的位置，再对全文生成 token embeddings，最后按位置截取并平均池化，得到带上下文的 chunk embedding。
- **详细步骤**：
1. 切出 chunks，为后续要存储到向量库的chunks做准备
``` python
from langchain.text_splitter import RecursiveCharacterTextSplitter  
...
2. 标准化每个chunk，去除特殊token,后面要让 chunk 自己 tokenize 后的 token，和完整文档里的 token 对齐
...
```
5. 计算 每个 chunk 在 doc_input_ids 里的 token 起止位置。
``` python
...
```
6. 生成全文 token embeddings,输出的是完整文档中每个 token 的 embedding
``` python
document_embeddings = document_to_token_embeddings(
    model,
    tokenizer,
    processed_doc,
    batch_size=1024
...
def document_to_token_embeddings(model, tokenizer, document, batch_size=4096):
    ...
    for i in trange(0, len(tokens), batch_size):
        ...
        batch_inputs = {
            ...
            for k, v in tokenized_document.items()
        }
...
7. 根据 每个 chunk 在 doc_input_ids 里的 token 起止位置 做 Late Chunking
``` python
late_embeddings = late_chunking(
    document_embeddings,
    [span_annotations]
)
def late_chunking(token_embeddings, span_annotation, max_length=None):
    ...
    for embeddings, annotations in zip(token_embeddings, span_annotation):
        ...
        for idx, (start, end) in enumerate(annotations):
            assert chunks[idx] == tokenizer.decode(
                doc_input_ids[start: end]
            ), f"idx={idx}, (start, end)={(start, end)}, chunks[idx]={chunks[idx]}, tokenizer.decode(doc_input_ids[start: end])={tokenizer.decode(doc_input_ids[start: end])}"
...
8. 自定义 EmbeddingFn 接入 Chroma,Chroma 默认会调用 embedding 函数，把文本重新 embedding。但这里不能让它重新普通 embedding。
``` python
